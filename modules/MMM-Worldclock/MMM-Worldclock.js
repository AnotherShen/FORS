...
Module.register("MMM-Worldclock",{
  ...
  clockFormat: function(c, index) {
    ...
    if (c.dateFormat == null || undefined ) {
      dateString = clock.format(this.config.dateFormat)
    } else {
      dateString = clock.format(c.dateFormat)
    }
    ...
    var subtextWrapper = document.createElement("div")
    subtextWrapper.className = "subtext"
    subtextWrapper.appendChild(dateWrapper)

    worldWrapper.appendChild(subtextWrapper)
